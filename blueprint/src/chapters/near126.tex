...
\begin{definition}[Near-$126$ literature input]
  ...
  Theorem~\ref{thm:external-theta5-order-data} and which supplies only order
  and difference-filtration data; finite torsion exclusions remain in the
  computation package.  The quadratic-construction fields supply the cited
  cell-calculus input for the internal choice comparison.  Its named
  \texttt{ExternalEvidence} field
...
\begin{proof}
  ...
  filtration and product data.  The resulting witness is passed explicitly
  to the Lean transport theorem; it is not a field of
  \texttt{AnyChoiceCriterion}.
\end{proof}
...
\begin{proposition}[Typed selected-product quotient compatibility]
  \label{prop:theta5-selected-product-quotient-compatibility}
  \uses{def:synthetic-catalogue-coherence,def:synthetic-lambda-quotient,def:lambda-rho-delta-triangle}
  \notready
  For the selected source product $\lambda\eta\theta_5^2$ in the fixed
  synthetic sphere, the untruncated product and its quotient images are typed
  through the same sphere, product, and quotient maps.  In particular, for
  every finite $r\ge1$, the map $\rho_{r,\infty}$ is equipped with the
  product-compatibility square needed to identify the image of the selected
  product with its finite counterpart.
\end{proposition}

\begin{proof}
  This is the concrete product/quotient coherence field of the selected
  synthetic catalogue; it is not inferred from cofiber naturality alone.
\end{proof}

\begin{proposition}[Transport from the total expression to finite quotients]
  \label{prop:theta5-total-quotient-transport}
  \uses{def:synthetic-lambda-quotient,def:lambda-rho-delta-triangle,prop:lambda-quotient-restriction-coherence,prop:theta5-selected-product-quotient-compatibility}
  \notready
...
\begin{proof}
  Apply the explicit order-two witness from
  Proposition~\ref{evidence:theta5-order-torsion}.  The current Lean
  choice-transport theorem takes correction-vanishing as an explicit premise;
  proving that premise for the fixed synthetic sphere remains the open
  obligation of Proposition~\ref{prop:theta5-choice-correction-vanishes}.
  Once supplied, expand the difference of the two squares to identify their
  $\lambda\eta$-images.  The finite-quotient equalities follow from
  Proposition~\ref{prop:theta5-total-quotient-transport}.  This argument
  does not assume the arbitrary-choice total differential identity; the
  source-choice identity is used in the next proposition instead.
\end{proof}
