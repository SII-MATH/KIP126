...
\begin{proposition}[Total differential identity for an arbitrary choice]
  ...
  \uses{def:near126-computation-input,def:synthetic-theta5-choice,lem:theta5-moore-extension,thm:external-bx-quadratic-cell-calculus}
  \notready
  ...
  This node is the internal Section~7 instantiation of the published uniform
  cell calculus, not an algebraic inference from order two and not an external
  arbitrary-choice identity.
\end{proposition}

\begin{proof}
  Use Lemma~\ref{lem:theta5-moore-extension} to choose
  $\widehat\theta_5$.  Instantiate the three fields of
  Theorem~\ref{thm:external-bx-quadratic-cell-calculus}: the quadratic map sends
  $Q_0(a)$ to $\theta_5^2$ and $Q_0(b)$ to the fixed class $h_6^2$, while its
  source satisfies
  $\delta_1(Q_0(b))=\lambda\eta Q_0(a)$.  Naturality of $\delta_1$ under this
  same quadratic map gives the displayed identity.  No comparison of
  $(\theta_5^{\mathrm{src}}+x)^2$ with
  $(\theta_5^{\mathrm{src}})^2$ is used, so order two is not being asked to
  kill unrecorded cross or square terms.
\end{proof}

\begin{proposition}[Choice independence of the inductive expression]
  \label{prop:theta5-choice-independence}
  \uses{def:near126-computation-input,def:synthetic-theta5-choice,evidence:theta5-order-torsion,prop:theta5-total-differential-choice-comparison}
  \notready
...
\begin{proof}
  ...
  products equal the same fixed total differential $\delta_1(h_6^2)$.
  Naturality of the quotient maps then gives equality in every finite quotient.
