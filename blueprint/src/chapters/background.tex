...
\begin{definition}[Complete and separated decreasing filtration]
  ...
  \uses{def:core-filtration,def:core-exhaustive-filtration,def:core-filtration-completion-witness}
  \notready
  ...
  complete, separated, and convergence interface remains pending.  A
  convergence witness must also record the regularity or boundedness condition
...
\begin{definition}[Selected-page comparison with a bounded complete abutment]
  ...
  records its endpoint boundary data, a graded abutment with a decreasing
  filtration, and degreewise boundedness.  It derives a canonical completion
  witness in every grading degree from the bounded-above component.  It also
  chooses, for every bidegree $(p,q)$, a page at least $2$ and an isomorphism
  from that page object to the indicated associated-graded piece of the
  abutment.  The boundedness field yields compiled proofs that the abutment
  filtration is exhaustive and eventually zero.
...
\begin{definition}[Functoriality of endpoint extensions]
  ...
  not claim a packaged Mathlib spectral-sequence morphism or pagewise
  naturality for the canonical $E_2$ sequences.
\end{definition}
