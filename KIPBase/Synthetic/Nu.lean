...
/-- KIP §3.1, Definition 3.8 (the ν functor): The ν functor from the stable
    homotopy category to synthetic spectra.
    Axiomatized: ν preserves finite colimits and is lax symmetric monoidal. -/
axiom nu : 𝒮 ⥤ Syn

/-- KIP §3.1, Definition 3.8: ν is an additive functor. -/
axiom nu_additive : Functor.Additive (nu 𝒮 Syn)

/-- KIP §3.1, Definition 3.8: ν preserves the zero object. -/
axiom nu_zero : (nu 𝒮 Syn).obj 0 ≅ (0 : Syn)

/-! ### Compatibility with suspension -/

/-- KIP §3.1, Remark after Definition 3.8: ν(ΣX) ≅ Σ^{1,1} ν(X). The ν
...
/-- KIP §3.1: ν intertwines the ℤ-shift with diagonal biShift:
...
/-- KIP §3.1, Axiom 3.1 (correct form): ν sends cofiber sequences to
