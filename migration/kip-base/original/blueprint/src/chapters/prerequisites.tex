...
\begin{definition}\label{prereq:def:ss-data}
    \lean{KIPBase.SpectralSequence.SSData}\leanok
...
\begin{definition}\label{prereq:def:spectral-sequence}
    ...
    \lean{KIPBase.SpectralSequence.SpectralSequence}\leanok
...
\begin{definition}\label{prereq:def:einfty-data}
    \uses{prereq:def:ss-data}
    \lean{KIPBase.SpectralSequence.SSData.eInfty}\leanok
...
\begin{definition}\label{prereq:def:ss-morphism}
    ...
    \lean{KIPBase.SpectralSequence.SpectralSequenceMorphism}\leanok
...
\begin{definition}\label{prereq:def:convergence}
    ...
    \lean{KIPBase.SpectralSequence.Convergence}\leanok
...
\begin{definition}\label{prereq:def:converging-ss-morphism}
    ...
    \lean{KIPBase.SpectralSequence.ConvergenceMorphism}\leanok
...
\begin{definition}\label{prereq:def:stem}
    ...
    \lean{KIPBase.SpectralSequence.Convergence.stemDegree}\leanok
...
\begin{definition}\label{prereq:def:filtration-degree}
    \uses{prereq:def:convergence}
    \lean{KIPBase.SpectralSequence.Convergence.filtrationDegree}\leanok
...
\begin{definition}\label{prereq:def:detect}
    \uses{prereq:def:convergence}
    \lean{KIPBase.SpectralSequence.Detects}\leanok
...
\begin{proposition}\label{prereq:prop:detect-zero}
    ...
    \lean{KIPBase.SpectralSequence.detect_zero}\leanok
...
\begin{proposition}\label{prereq:prop:detect-difference}
    \uses{prereq:def:detect}
    \lean{KIPBase.SpectralSequence.detect_difference}\leanok
...
\begin{definition}\label{prereq:def:filtration}
    \lean{KIPBase.SpectralSequence.Filtration}\leanok
...
\begin{definition}\label{prereq:def:filtered-morphism}
    ...
    \lean{KIPBase.SpectralSequence.FilteredMorphism}\leanok
...
\begin{definition}\label{prereq:def:filtered-complex}
    \uses{prereq:def:filtration}
    \lean{KIPBase.SpectralSequence.FilteredComplex}\leanok
...
\begin{proposition}\label{prereq:prop:homology-filtration}
    ...
    \lean{KIPBase.SpectralSequence.FilteredComplex.homologyFiltration}\leanok
...
\begin{definition}\label{prereq:def:associated-graded}
    \uses{prereq:def:filtration}
    \lean{KIPBase.SpectralSequence.Filtration.associatedGraded}\leanok
...
\begin{definition}\label{prereq:def:associated-graded-complex}
    ...
    \lean{KIPBase.SpectralSequence.FilteredComplex.assocGraded}\leanok
...
\begin{definition}\label{prereq:def:exhaustive}
    \uses{prereq:def:filtered-complex}
    \lean{KIPBase.SpectralSequence.Filtration.IsExhaustive}\leanok
...
\begin{definition}\label{prereq:def:hausdorff}
    \uses{prereq:def:filtered-complex}
    \lean{KIPBase.SpectralSequence.Filtration.IsHausdorff}\leanok
...
\begin{definition}\label{prereq:def:bounded-filtration}
    ...
    \lean{KIPBase.SpectralSequence.FilteredComplex.IsBounded}\leanok
...
\begin{definition}\label{prereq:def:filtered-complex-ssdata}
    ...
    \lean{KIPBase.SpectralSequence.FilteredComplex.toSSData}\leanok
...
\begin{proposition}\label{prereq:prop:filtered-complex-ssdata-wd}
    ...
    \lean{KIPBase.SpectralSequence.FilteredComplex.pageDifferential_comp}\leanok
...
\begin{theorem}\label{prereq:thm:filtered-complex-ss}
    ...
    \lean{KIPBase.SpectralSequence.FilteredComplex.toSpectralSequence}\leanok
...
\begin{theorem}\label{prereq:thm:filtered-complex-convergence}
    ...
    \lean{KIPBase.SpectralSequence.FilteredComplex.weakConvergence}\leanok
...
\begin{definition}\label{prereq:def:essential-differential}
    \uses{prereq:def:spectral-sequence}
    \lean{KIPBase.SpectralSequence.IsEssentialAt}\leanok
...
\begin{definition}\label{prereq:def:differential-data}
    ...
    \lean{KIPBase.SpectralSequence.DifferentialDatum}\leanok
...
\begin{definition}\label{prereq:def:crossing}
    ...
    \lean{KIPBase.SpectralSequence.HasCrossingAt}\leanok
...
\begin{axiom}\label{prereq:ax:no-crossing-iff}
    \uses{prereq:def:crossing,prereq:ax:f-ext-characterization,prereq:ax:f-ext-inessential}
    \notready
...
\end{axiom}
\begin{proof}\leanok
    \emph{Only if.} Suppose the extension has no crossing in $\mathrm{Fil} \ge p$. Assume for contradiction that there exists $[x] \in \{x\}$ with $\mathrm{Fil}(f_A[x]) \ge p$ but $f_A[x] \notin \{y\}$. Let $y'$ be the element of $E_\infty(V_2)$ detecting $f_A[x]$, so $f_A[x] \in \{y'\}$ with $\mathrm{Fil}(y') \ge p$. By Proposition~\ref{prereq:ax:f-ext-characterization} there is an $f$-extension from $x$ to $y'$. By Proposition~\ref{prereq:ax:f-ext-inessential}~(2), $y'$ (if $\mathrm{Fil}(y) > \mathrm{Fil}(y')$) or $y - y'$ (if $\mathrm{Fil}(y) = \mathrm{Fil}(y')$) is hit by a shorter $d^f$-differential from some $x'$ with $\mathrm{Fil}(x') > \mathrm{Fil}(x)$. This gives a crossing that hits $\mathrm{Fil} \ge p$, contradicting the assumption.

    \emph{If.} Suppose the extension has a crossing: there exist $x' \in E_\infty^{s+a}(V_1)$ with $a > 0$ and essential $d_{n-a}^f(x') = y'$ with $p \le \mathrm{Fil}(y') \le \mathrm{Fil}(y)$. By Proposition~\ref{prereq:ax:f-ext-characterization}, there exists $[x'] \in \{x'\}$ with $f_A([x']) \in \{y'\}$. Since $\mathrm{Fil}(x') > \mathrm{Fil}(x)$, the element $[x] + [x'] \in \{x\}$ (as $[x']$ lies in higher filtration). Then
    $$f_A([x] + [x']) = f_A([x]) + f_A([x']),$$
...
\begin{definition}\label{prereq:def:ess}
    ...
    \lean{KIPBase.SpectralSequence.BoundedExtensionSS}\leanok
...
\begin{axiom}\label{prereq:ax:ess-e0-iso}
...
\begin{definition}\label{prereq:def:f-extension-diff}
    ...
    \lean{KIPBase.SpectralSequence.HasFExtension}
...
\begin{axiom}\label{prereq:ax:ess-d0}
...
\begin{notation}\label{prereq:not:representatives}
    \uses{prereq:def:detect}
    \lean{KIPBase.SpectralSequence.DetectionSet}\leanok
...
\begin{axiom}\label{prereq:ax:f-ext-characterization}
...
\begin{axiom}\label{prereq:ax:f-ext-inessential}
    \uses{prereq:ax:f-ext-characterization,prereq:not:representatives}
    \notready
    ...
    \end{enumerate}
\end{axiom}
\begin{proof}\leanok Part~(1) follows from the definition of the page boundaries in the filtered complex construction. Parts~(2a) and~(2b) follow from Part~(1) and Proposition~\ref{prereq:ax:f-ext-characterization}.\end{proof}

\begin{axiom}\label{prereq:ax:ess-exactness}
    \uses{prereq:ax:f-ext-characterization,prereq:ax:composition-zero,prereq:not:representatives}
    \notready
...
\begin{definition}\label{prereq:def:truncated-ss}
    ...
    \lean{KIPBase.SpectralSequence.Filtration.truncatedFiltration}\leanok
...
\begin{proposition}\label{prereq:prop:truncations-inverse-system}
    ...
    \lean{KIPBase.SpectralSequence.Filtration.truncationTransition_comp}\leanok
...
\begin{definition}\label{prereq:def:unbounded-ess}
    ...
    \lean{KIPBase.SpectralSequence.UnboundedExtensionSS}
...
\begin{proposition}\label{prereq:prop:unbounded-ess-stabilization}
    ...
    \lean{KIPBase.SpectralSequence.truncatedESS_pageStabilization}
...
\begin{definition}\label{prereq:def:filtration-completion}
    ...
    \lean{KIPBase.SpectralSequence.Filtration.completion}\leanok
...
\begin{axiom}\label{prereq:ax:unbounded-ess-convergence}
    \uses{prereq:def:unbounded-ess,prereq:def:filtration-completion,prereq:prop:unbounded-ess-stabilization}
    \lean{KIPBase.SpectralSequence.UnboundedExtensionSS.weakConvergence}
...
\begin{axiom}\label{prereq:ax:four-spectra}
    \uses{prereq:ax:no-crossing-iff,prereq:ax:f-ext-characterization}
    \notready
...
\begin{proof}\leanok
    ...
    If the differential in~(2) has no crossing, by Proposition~\ref{prereq:ax:no-crossing-iff} we can pick $[x] \in \{x\}$ such that $p_A[x] \in \{z\}$; by Proposition~\ref{prereq:ax:f-ext-characterization}, $f_A[x] \in \{y'\}$ for some $y'$, but the no-crossing condition forces $f_A[x] \in \{y\}$. Symmetrically, if the differential in~(1) has no crossing, we pick $[x]$ with $f_A[x] \in \{y\}$ and the no-crossing forces $p_A[x] \in \{z\}$.

    By~(5) and Proposition~\ref{prereq:ax:no-crossing-iff}~(2), since $d_{k-1}^q y = 0$ has no crossing, $\mathrm{Fil}(q_A f_A[x]) \ge s + n + k$. Since the diagram commutes on target groups, $g_A p_A[x] = q_A f_A[x]$, so $\mathrm{Fil}(g_A p_A[x]) \ge s + n + k$.

    Combining with~(4) ($d_l^g(z) = w$ has no crossing that hits $\mathrm{Fil} \ge s + n + k$) and $p_A[x] \in \{z\}$, Proposition~\ref{prereq:ax:no-crossing-iff} gives $g_A p_A[x] \in \{w\}$.

    Therefore $q_A f_A[x] \in \{w\}$, and since $f_A[x] \in \{y\}$, there is a $q$-extension from $y$ to $w$ by Proposition~\ref{prereq:ax:f-ext-characterization}.
\end{proof}

\begin{axiom}\label{prereq:ax:four-spectra-cor}
    \uses{prereq:ax:four-spectra}
    \notready
    In the setting of Theorem~\ref{prereq:ax:four-spectra}, if condition~(4) is strengthened to ``$d_l^g(z) = w$ has no crossing'' (and condition~(5) is dropped), then $d_{m+l-n}^q(y) = w$.
\end{axiom}
\begin{proof}\leanok If $d_l^g(z) = w$ has no crossing, then in particular it has no crossing that hits $\mathrm{Fil} \ge s + n + k$ for any $k$. Taking $k = 1$ and observing that $d_0^q y = 0$ trivially has no crossing, the result follows from Theorem~\ref{prereq:ax:four-spectra}.\end{proof}

\begin{axiom}\label{prereq:ax:triangle-map}
    \uses{prereq:ax:four-spectra-cor}
    \lean{KIPBase.SpectralSequence.essCommutativity_triangle}\leanok
...
\end{axiom}
\begin{proof}\leanok This is the special case of Corollary~\ref{prereq:ax:four-spectra-cor} with $V_3 = V_4$ and $g = \mathrm{id}_{V_3}$: since $d_0^g(z) = z$ and the identity extension always has no crossing, the conditions are satisfied with $l = 0$.\end{proof}

\begin{axiom}\label{prereq:ax:composition-ext}
    \uses{prereq:ax:four-spectra-cor}
    \notready
...
\end{axiom}
\begin{proof}\leanok This is the special case of Corollary~\ref{prereq:ax:four-spectra-cor} with $V_1 = V_2$ and $f = \mathrm{id}_{V_1}$: since $d_0^f(x) = x$ with $n = 0$ and the identity extension has no crossing, the conditions give $d_{m+l-0}^q(y) = d_{m+l}^q(x) = w$.\end{proof}

\begin{axiom}\label{prereq:ax:composition-zero}
    \uses{prereq:ax:four-spectra-cor,prereq:ax:f-ext-characterization}
    \notready
...
\begin{proof}\leanok
    ...
    0 @>>> V_3
    \end{CD}$$
    with $p = 0$ and $g \circ f = 0$. All extensions from $x$ to $0$ are trivial and have no crossing. By Corollary~\ref{prereq:ax:four-spectra-cor}, $d_m^g(y) = 0$.

    \emph{Alternative proof.} By Proposition~\ref{prereq:ax:f-ext-characterization}, there exists $[x] \in \{x\}$ with $f_A[x] \in \{y\}$. Let $[y] = f_A[x]$. Then $g_A([y]) = g_A(f_A([x])) = 0$. Hence $y$ is a permanent $d^g$-cycle.
\end{proof}

\begin{axiom}\label{prereq:ax:ess-naturality}
    \uses{prereq:def:ess,prereq:ax:four-spectra-cor}
    \lean{KIPBase.SpectralSequence.essCommutativity_induces_map}\leanok
    Consider a homotopy commutative diagram of converging spectral sequences
    $$\begin{CD}
    V_1 @>{f}>> V_2 \\
    @V{p}VV @VV{q}V \\
...
\end{axiom}
\begin{proof}\leanok
    Since ${}^{p}\!E_0 = {}^{p}\!E_r$ and ${}^{q}\!E_0 = {}^{q}\!E_r$, the differentials $d_r^p$ and $d_r^q$ have no crossings. By Corollary~\ref{prereq:ax:four-spectra-cor}, $d_0^g \circ d_r^p = d_r^q \circ d_0^f$ and therefore $(d_r^p, d_r^q)$ induces a map from ${}^{f}\!E_1$ to ${}^{g}\!E_1$. Inductively, applying Corollary~\ref{prereq:ax:four-spectra-cor} shows that $(d_r^p, d_r^q)$ induces a map from ${}^{f}\!E_n$ to ${}^{g}\!E_n$ and $d_n^g \circ d_r^p = d_r^q \circ d_n^f$ for all $n$.
\end{proof}
...
\begin{definition}\label{prereq:def:cofiber-sequence}
    ...
    \lean{KIPBase.StableHomotopy.HoCofiberSequence}\leanok
...
\begin{definition}\label{prereq:def:functorial-cofiber}
    ...
    \lean{KIPBase.StableHomotopy.HasFunctorialCofiber}\leanok
...
\begin{definition}\label{prereq:def:closed-symmetric-tensor-triangulated}
    ...
    \lean{KIPBase.StableHomotopy.ClosedSymmetricTensorTriangulated}\leanok
...
\begin{definition}\label{prereq:def:tensor-triangulated-functorial-cofiber}
    ...
    \lean{KIPBase.StableHomotopy.TensorTriangulatedCatWithFunctorialCofiber}\leanok
    ...
    \begin{enumerate}
        ...
        \item A \emph{tensor-cofiber exchange}: for any morphism $f: X \to Y$ and any object $W$, a natural isomorphism
...
\begin{definition}\label{prereq:def:stable-homotopy-category}
    ...
    \lean{KIPBase.StableHomotopy.StableHomotopyCategory}\leanok
    The \emph{stable homotopy category} $\mathcal{S}$ is a closed symmetric tensor triangulated category with functorial cofiber (Definition~\ref{prereq:def:tensor-triangulated-functorial-cofiber}). Its objects are called \emph{spectra}.
    % NOTE: The statement "Homotopy pullback squares and homotopy pushout squares in $\mathcal{S}$ coincide"
    % has been removed, as it cannot be stated in the current axiomatic framework.
    % Pullbacks are not defined. The pushout is defined separately below (Definition~\ref{prereq:def:pushout}).
\end{definition}
...
\begin{definition}\label{prereq:def:pushout}
    \uses{prereq:def:stable-homotopy-category,prereq:def:cofiber-sequence,prereq:def:functorial-cofiber}
    Given morphisms $f: X \to Y$ and $g: X \to Z$ in $\mathcal{S}$, the \emph{pushout} of the diagram $Y \xleftarrow{f} X \xrightarrow{g} Z$ is the cofiber $C(f, g)$ of the canonical morphism $(f, g): X \to Y \vee Z$, where $Y \vee Z$ denotes the coproduct (wedge sum) in $\mathcal{S}$.
\end{definition}

\begin{definition}\label{prereq:def:sphere-spectrum}
    \uses{prereq:def:stable-homotopy-category}
    \lean{KIPBase.StableHomotopy.SphereSpectrum}\leanok
...
\begin{definition}\label{prereq:def:sphere-Sn}
    ...
    \lean{KIPBase.StableHomotopy.Sphere}\leanok
...
\begin{definition}\label{prereq:def:homotopy-groups}
    ...
    \lean{KIPBase.StableHomotopy.HomotopyGroup}\leanok
...
\begin{proposition}\label{prereq:prop:homotopy-graded-abelian}
    ...
    \lean{KIPBase.StableHomotopy.HomotopyGroups}\leanok
...
\begin{definition}\label{prereq:def:smash-product}
    ...
    \lean{KIPBase.StableHomotopy.smashSpheres_iso}\leanok
...
\begin{definition}\label{prereq:def:mapping-spectra}
    ...
    \lean{KIPBase.StableHomotopy.MappingSpectrum}\leanok
...
\begin{proposition}\label{prereq:prop:les-homotopy}
    ...
    \lean{KIPBase.StableHomotopy.les_homotopy_exact_f}\leanok
...
\begin{axiom}\label{prereq:ax:eilenberg-maclane}
    \uses{prereq:def:stable-homotopy-category}
    \lean{KIPBase.StableHomotopy.EilenbergMacLane}\leanok
...
\begin{axiom}\label{prereq:ax:eilenberg-maclane-homotopy}
    \uses{prereq:ax:eilenberg-maclane,prereq:def:homotopy-groups}
    \lean{KIPBase.StableHomotopy.HF2_pi0}\leanok
...
\begin{definition}\label{prereq:def:mod2-cohomology}
    \uses{prereq:ax:eilenberg-maclane,prereq:def:mapping-spectra}
    \lean{KIPBase.StableHomotopy.Mod2CohomologyTotal}\leanok
    The \emph{mod~2 cohomology} of a spectrum $X$ is
    $$H^n(X; \bF_2) = [\Sigma^n X, \HF].$$
\end{definition}

\begin{definition}\label{prereq:def:mod2-homology}
    \uses{prereq:ax:eilenberg-maclane,prereq:def:smash-product,prereq:def:homotopy-groups}
    \lean{KIPBase.StableHomotopy.Mod2HomologyTotal}\leanok
...
\begin{axiom}\label{prereq:ax:universal-coefficient}
    ...
    \lean{KIPBase.StableHomotopy.universal_coefficient}\leanok
...
\begin{axiom}\label{prereq:ax:adams-ss}
    ...
    \lean{KIPBase.StableHomotopy.AdamsSS}\leanok
    The \emph{$\HF$-Adams spectral sequence} is a functor from $\mathcal{S}^{op} \times \mathcal{S}$ to the category of bigraded spectral sequences. For spectra $X$ and $Y$, it produces a spectral sequence $\{E_r^{s,t}(X, Y), d_r\}$ with $r \ge 2$ and differentials
...
\begin{axiom}\label{prereq:ax:adams-e2-order2}
    ...
    \lean{KIPBase.StableHomotopy.adams_2torsion}\leanok
...
\begin{axiom}\label{prereq:ax:connected-homology-vanishing}
    ...
    \lean{KIPBase.StableHomotopy.connected_homology_vanishing}\leanok
...
\begin{axiom}\label{prereq:ax:finite-type-char}
    ...
    \lean{KIPBase.StableHomotopy.finite_spectra_char}\leanok
...
\begin{axiom}\label{prereq:ax:adams-filtration}
    \uses{prereq:ax:adams-ss,prereq:def:filtration,prereq:def:homotopy-groups}
    \lean{KIPBase.StableHomotopy.InAdamsFiltration}\leanok
...
\begin{definition}\label{prereq:def:adams-filtration-maps}
    \uses{prereq:ax:adams-filtration,prereq:def:mod2-homology,prereq:def:finite-spectra}
    \lean{KIPBase.StableHomotopy.AF}\leanok
...
\begin{axiom}\label{prereq:ax:adams-filtration-characterization}
    \uses{prereq:def:adams-filtration-maps}
    \lean{KIPBase.StableHomotopy.adamsFiltration_functorial}\leanok
    A map $f: X \to Y$ with $\AF(f) = k$ maps $F^s \pi_* X$ into $F^{s+k} \pi_* Y$ for all $s$.
\end{axiom}

\begin{axiom}\label{prereq:ax:af-vanishing}
    \uses{prereq:def:adams-filtration-maps,prereq:def:ess,prereq:ax:f-ext-characterization}
    \notready
    If $\mathrm{AF}(f) = k$, then $d_i^f = 0$ for $i < k$ in the $f$-extension spectral sequence.
\end{axiom}
\begin{proof}\leanok By Proposition~\ref{prereq:ax:f-ext-characterization}, $d_i^f(x) = y$ requires $[x] \in \{x\}$ with $f_A[x] \in \{y\}$, which forces $\mathrm{Fil}(f_A[x]) \ge s + i$. But $\mathrm{AF}(f) = k$ means $f_A$ raises filtration by at least $k$, so $\mathrm{Fil}(f_A[x]) \ge s + k > s + i$ for $i < k$, hence $y = 0$.\end{proof}
...
\begin{axiom}\label{prereq:ax:odd-order-af-infinity}
    \uses{prereq:def:adams-filtration-infinity}
    \lean{KIPBase.StableHomotopy.af_odd_order_infinite}\leanok
    If $f$ has odd order in $[X,Y]$, then $f$ has Adams filtration $\infty$.
\end{axiom}

\begin{axiom}\label{prereq:ax:not-odd-order-af-finite}
    \uses{prereq:def:adams-filtration-infinity}
    \lean{KIPBase.StableHomotopy.af_not_odd_order_finite}\leanok
...
\begin{axiom}\label{prereq:ax:adams-weak-convergence}
    ...
    \lean{KIPBase.StableHomotopy.adamsConvergence_bigraded}\leanok
...
\begin{axiom}\label{prereq:ax:adams-separated}
    \uses{prereq:ax:adams-weak-convergence,prereq:ax:odd-order-af-infinity}
    \lean{KIPBase.StableHomotopy.adams_separated}\leanok
...
\begin{axiom}\label{prereq:ax:adams-boundedness}
    \uses{prereq:ax:adams-separated,prereq:def:finite-type}
    \lean{KIPBase.StableHomotopy.adams_boundedness}\leanok
    Suppose $X$ is of finite type. Then:
    \begin{enumerate}
        \item If additionally $H_i(X; \bQ) = 0$ for some $i$, then the Adams filtration on $\pi_i(X) / \{\text{odd-order elements}\}$ is bounded.
        \item All subquotients of the Adams filtration on $\pi_i(X) / \{\text{odd-order elements}\}$ are finite groups.
    \end{enumerate}
\end{axiom}
...
\begin{definition}\label{prereq:def:synthetic-category}
    ...
    \lean{KIPBase.Synthetic.SyntheticCategory}\leanok
...
\begin{definition}\label{prereq:def:bigraded-suspension}
    ...
    \lean{KIPBase.Synthetic.SyntheticCategory.biShift}\leanok
...
\begin{definition}\label{prereq:def:lambda}
    ...
    \lean{KIPBase.Synthetic.SyntheticCategory.lam}\leanok
...
\begin{definition}\label{prereq:def:lambda-power}
    ...
    \lean{KIPBase.Synthetic.lambdaPow}\leanok
...
\begin{axiom}\label{prereq:ax:zlambda-enrichment}
    \uses{prereq:def:lambda,prereq:def:synthetic-category}
    \lean{KIPBase.Synthetic.zlambda_enrichment}\leanok
...
\begin{definition}\label{prereq:def:bigraded-spheres}
    \uses{prereq:def:bigraded-suspension,prereq:def:synthetic-category}
    \lean{KIPBase.Synthetic.Smn}\leanok
...
\begin{proposition}\label{prereq:prop:bishift-tensor-sphere}
    ...
    \lean{KIPBase.Synthetic.biShift_eq_tensor_Smn}\leanok
...
\begin{definition}\label{prereq:def:synthetic-homotopy-groups}
    ...
    \lean{KIPBase.Synthetic.BiHom}\leanok
    The \emph{synthetic homotopy groups} of a synthetic spectrum $X$ are
    $$\pi_{m,n}(X) = [S^{m,n}, X].$$
    These form a bigraded abelian group with a natural $\mathbb{Z}[\lambda]$-module structure (Proposition~\ref{prereq:ax:zlambda-enrichment}).
\end{definition}
...
\begin{proposition}\label{prereq:prop:synthetic-homotopy-shift}
    ...
    \lean{KIPBase.Synthetic.susp_invariance}\leanok
...
\begin{definition}\label{prereq:def:lambda-action}
    \uses{prereq:def:synthetic-homotopy-groups,prereq:def:lambda,prereq:ax:zlambda-enrichment}
    \lean{KIPBase.Synthetic.lambdaAction}\leanok
...
\begin{axiom}\label{prereq:ax:syn-adams-ss}
    ...
    \lean{KIPBase.Synthetic.SynAdamsSS}\leanok
    The \emph{synthetic Adams spectral sequence} is a functor from $h\mathrm{Syn}$ to the category of 3-graded spectral sequences. For a synthetic spectrum $X \in h\mathrm{Syn}$, it produces a spectral sequence $\SynSS_r^{s,t,w}(X)$ with differentials
    $$d_r: \SynSS_r^{s,t,w} \to \SynSS_r^{s+r, t+r-1, w}.$$
    For a finite spectrum $X \in \mathcal{S}^{\mathrm{fin}}$, the synthetic Adams spectral sequence for $\nu X$ converges to $\pi_{*,*}(\nu X)$.
\end{axiom}
...
\begin{axiom}\label{prereq:ax:nu-functor}
    \uses{prereq:def:stable-homotopy-category,prereq:def:synthetic-category}
    \lean{KIPBase.Synthetic.nu}\leanok
...
\begin{axiom}\label{prereq:ax:nu-susp}
    \uses{prereq:ax:nu-functor,prereq:def:bigraded-suspension}
    \lean{KIPBase.Synthetic.nu_susp}\leanok
...
\begin{theorem}\label{prereq:thm:nu-shift-bishift}
    ...
    \lean{KIPBase.Synthetic.nu_shift_biShift}\leanok
...
\begin{axiom}\label{prereq:ax:nu-cofiber-ses}
    \uses{prereq:ax:nu-functor,prereq:def:cofiber-sequence,prereq:def:mod2-homology}
    \lean{KIPBase.Synthetic.nu_cofiber_ses}\leanok
...
\begin{remark}\label{prereq:rem:nu-cofiber-cohomological}
    ...
    \lean{KIPBase.Synthetic.nu_cofiber_ses_cohomological}\leanok
...
\begin{axiom}\label{prereq:ax:rigid}
    ...
    \lean{KIPBase.Synthetic.rigidity_degeneration}\leanok
...
\begin{axiom}\label{prereq:ax:lambda-bockstein}
    ...
    \lean{KIPBase.Synthetic.lambda_bockstein_iso}\leanok
...
\begin{proposition}\label{prereq:prop:syn-einfty-nuX}
    ...
    \lean{KIPBase.Synthetic.einfty_nuX}\leanok
...
\begin{axiom}\label{prereq:ax:syn-einfty-mod-lambda}
    \uses{prereq:ax:rigid,prereq:def:lambda}
    \lean{KIPBase.Synthetic.einfty_nuX_mod_lambda}\leanok
...
\begin{axiom}\label{prereq:ax:synthetic-lift}
    ...
    \lean{KIPBase.Synthetic.synthetic_lift}\leanok
    Let $X, Y \in \mathcal{S}^{\mathrm{fin}}$ be finite spectra. If a map $f: X \to Y$ has Adams filtration $\AF(f) = k$, then there exists a factorization
    $$\begin{CD}
    @. \Sigma^{0,-k}\nu Y \\
    @. @VV{\lambda^k}V \\
    \nu X @>{\nu f}>> \nu Y
...
\begin{axiom}\label{prereq:ax:lambda-g-equals-f}
    \uses{prereq:ax:nu-cofiber-ses,prereq:ax:synthetic-lift,prereq:def:lambda,prereq:def:finite-spectra}
    Let $X, Y \in \mathcal{S}^{\mathrm{fin}}$ be finite spectra and let $f: X \to Y$ be a map that induces the zero map on $\HF$-homology: $H_*(f; \bF_2) = 0$. Let
    $$W \xrightarrow{i} X \xrightarrow{f} Y \xrightarrow{h} \Sigma W$$
    ...
    $$\nu(\Sigma^{-1}Y) \xrightarrow{g} \nu W \xrightarrow{\nu i} \nu X,$$
    where $g$ denotes the boundary map. Then:
    $$\lambda g = \nu f,$$
    i.e., the boundary map $g$ composed with $\lambda$ equals the synthetic image of $f$.
\end{axiom}

\begin{axiom}\label{prereq:ax:syn-distinguished-triangle}
    \uses{prereq:ax:nu-cofiber-ses,prereq:ax:synthetic-lift,prereq:ax:lambda-g-equals-f}
    \lean{KIPBase.Synthetic.syn_distinguished_triangle}\leanok
    Let $X, Y, Z \in \mathcal{S}^{\mathrm{fin}}$ be finite spectra. Suppose that $X \xrightarrow{f} Y \xrightarrow{g} Z \xrightarrow{h} \Sigma X$ is a distinguished triangle of spectra with $\AF(h) > 0$, and consequently a short exact sequence on $\HF$-homology
    ...
    such that $\nu h = \lambda \hat{h}$. The last relation $\nu h = \lambda \hat{h}$ follows from Axiom~\ref{prereq:ax:lambda-g-equals-f}.
\end{axiom}

\begin{notation}\label{prereq:not:fhat}
    \uses{prereq:ax:syn-distinguished-triangle,prereq:ax:lambda-g-equals-f}
    \lean{KIPBase.Synthetic.eHat}\leanok
    For a map $f: X \to Y$ which is part of a distinguished triangle $X \xrightarrow{f} Y \xrightarrow{g} Cf \to \Sigma X$, define
    $$e(f) = \begin{cases}
        0 & \text{if } \AF(f) = 0, \\
        1 & \text{if } \AF(f) > 0.
    \end{cases}$$
    When $\AF(f) = 0$, we also denote $\nu f$ by $\hat{f}$. In both cases, we have $\hat{f}: \Sigma^{0,e(f)}\nu X \to \nu Y$ and $\nu f = \lambda^{e(f)}\hat{f}$. Furthermore, $C\hat{f} \simeq \Sigma^{0,-e(g)}\nu Cf$ (by Axiom~\ref{prereq:ax:lambda-g-equals-f}).
\end{notation}

\begin{axiom}\label{prereq:ax:fhat-triangle}
    \uses{prereq:ax:syn-distinguished-triangle,prereq:not:fhat,prereq:ax:lambda-g-equals-f}
    \lean{KIPBase.Synthetic.fhat_triangle}\leanok
    ...
    The construction uses Axiom~\ref{prereq:ax:lambda-g-equals-f} to establish the $\lambda$-relations between the classical and synthetic connecting maps.
\end{axiom}
