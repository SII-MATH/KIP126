...
KIP Blueprint §0.2.3 (Cohomology): Mod 2 cohomology and homology theories
...
(see `KIPBase.StableHomotopy.Adams`).
-/
...
/-- KIP §0.2.3, Axiom `prereq:ax:eilenberg-maclane-homotopy` (n = 0 case):
...
KIP Blueprint §0.2.3: The Steenrod algebra A = π_*(F(HF₂, HF₂)) is the
...
KIP Blueprint §0.2.3, Definition `prereq:def:mod2-cohomology`:
The mod 2 cohomology of a spectrum X is defined as
  H^n(X; F₂) = [Σⁿ X, HF₂] = Hom(Σⁿ X, HF₂).
-/

/-- KIP §0.2.3, Definition `prereq:def:mod2-cohomology`:
...
KIP Blueprint §0.2.3, Definition `prereq:def:mod2-homology`:
The mod 2 homology of a spectrum X is defined as
  H_n(X; F₂) = πₙ(HF₂ ∧ X) = πₙ(HF₂ ⊗ X).
-/

/-- KIP §0.2.3, Definition `prereq:def:mod2-homology`:
...
KIP Blueprint §0.2.3, Definition `prereq:def:mapping-spectra`:
...
KIP Blueprint §0.2.3: Cohomology is representable by HF₂ via the
...
/-- KIP Universal Coefficient Theorem (§0.2.3): mod 2 cohomology is the
