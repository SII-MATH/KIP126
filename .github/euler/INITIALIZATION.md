...
| `auto-merge.yml` | Uses the repository-local `scripts/merge_gate.py` on successful `pr-build`, verified scoreboard updates, or manual dispatch; it never runs a reviewer directly. Without a native queue, every event shares one global coordinator and a persistent `merge-train-head` label. | It reads exact-head KIP126 statuses and trusted scoreboard metadata, routes red `scope` to human review, advances only the single train head, updates its branch with `MERGE`, and enables final native auto-merge with `SQUASH`; no App credential or cross-repository backend is used. |
