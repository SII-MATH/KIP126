...
\begin{proposition}[The $h_4$ classical--synthetic $d_2$ comparison]
  ...
  \notready
  The current Lean slice exposes the three-graded indices, the actual
  Mathlib page/differential interface, and the $(s,t,w)\mapsto(s,t)$ degree
  adapter.  It does not yet construct a typed multiplication-by-$λ$ action
  (with grading and page/multiplication coherence), nor canonical
  classical--synthetic page morphisms from representatives.  Consequently
  `SyntheticAdamsSS.lambdaMap` and
  `H₄DifferentialComparison` still expose these maps and their differential
  square as input data; they must not be read as a constructed
  $\lambda\cdot h_0h_3^2$ correspondence.  The proposition remains `notready`
  until the upstream synthetic spectrum/action and canonical page-map data
  are available.
\end{proposition}

\begin{proof}
  The AIM-5 catalogue wrapper supplies the classical statement and the
  index adapter proves the displayed degree equalities.  Mathlib's
  `CategoryTheory.SpectralSequence` supplies page objects, differentials,
  and page-passage isomorphisms, but not a multiplicative $λ$ action or a
  heterogeneous canonical page map.  The missing typed action/coherence and
  representative-induced maps are therefore an upstream data-model blocker;
  no proof of the differential correspondence is claimed here.
\end{proof}
