...
% SOURCE: aimpaper/main.tex:1009--1021; Burklund--Xu Construction 7.7,
% reference/BurklundXu/paper.txt:3022--3035.  The project-level statement
% deliberately forgets the commutative-algebra structure and retains only the
% finite quotient maps and their triangle data.
\begin{proposition}[Coherence of quotient restrictions]
  ...
  \notready
  For every map of synthetic spectra $f:X\to Y$ and finite integers
  $0<k<n<m$, write
  $\bar f_j:X/\lambda^j\to Y/\lambda^j$ for the map induced by $\nu f$.
  The quotient restriction maps are natural:
  \[
    \rho^Y_{n,m}\circ\bar f_m
      =\bar f_n\circ\rho^X_{n,m}.
  \]
  They satisfy the identity and composition laws
  \[
    \rho^X_{n,n}=\operatorname{id},\qquad
    \rho^X_{k,n}\circ\rho^X_{n,m}=\rho^X_{k,m}.
  \]
  For the same triple, the canonical shifted $\lambda^{n-k}$ maps on the
  first and last terms, the identity on $X/\lambda^m$, and $\rho^X_{k,n}$ on
  the third term form a morphism from the finite
  $\lambda$--$\rho$--$\delta$ triangle at $(n,m)$ to the one at $(k,m)$.
  Thus multiplication and connecting maps commute with finite restriction,
  naturally in $f$.  This proposition asserts no map of $E_\infty$- or
  $\mathrm{CAlg}$-objects; the $m=\infty$ endpoint is handled separately by
  the complete-tower statements.
\end{proposition}

\begin{proof}
  The maps $\bar f_j$ and $\rho_{i,j}$ are induced by the compatible finite
  cofiber presentations of multiplication by powers of $\lambda$.  The
  functoriality of cofibers gives the displayed naturality square and the
  morphism of finite distinguished triangles; the first and last components
  are the shifted $\lambda^{n-k}$ maps in that cofiber diagram.  The identity
  and composition equations are the equalities of the corresponding quotient
  projections.  No multiplicative algebra structure is used.
\end{proof}
