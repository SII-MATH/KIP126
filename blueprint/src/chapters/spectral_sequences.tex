...
\begin{definition}[Synthetic tridegree and page-level conversion]
  ...
  degree $(0,0,-1)$, while the functor $\nu$ sends a classical bidegree
  $(s,t)$ to $(s,t,t)$.  These grading operations are implemented.  They do
...
\begin{definition}[Reindexed spectral-sequence morphism]
  ...
  \notready
  For a sequence $E$ graded by $J$, a sequence $E'$ graded by $I$, and a
  reindexing $\phi:I\to J$, a reindexed morphism specifies a page-index map,
  admissibility proofs, pagewise maps
  $E_r^{\phi(i)}\to (E'_{r'})^i$, and the
  equation saying that $\phi$ carries every $E'$ differential target to the
  corresponding $E$ differential target.  Its differential squares commute,
  and its maps on homology agree with the maps used in the successor-page
  square.

  The compiled classical--synthetic slice has the same page index on both
  sides and $\phi(s,t,w)=(s,t)$.  It stores page maps, homology maps,
  differential commutation, and successor-page commutation.  It does not yet
  prove that the stored homology map is induced by the stored page map, nor
  provide identities, composition, or the shifted reindexings
