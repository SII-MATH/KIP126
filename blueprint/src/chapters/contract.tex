...
% SOURCE: PROJECT_BOUNDARY.md, External inputs; docs/FORMALIZATION_SPEC.md:51--64.
...
% SOURCE: PROJECT_BOUNDARY.md:149--167; docs/FORMALIZATION_SPEC.md:50--69.
...
\begin{definition}[External evidence]
  ...
  \uses{def:source-ref,def:artifact-ref,def:evidence-metadata}
\end{definition}
...
\begin{definition}[Source availability]
  ...
  \leanok
  A source-availability value records the four independently checked flags for
  metadata, PDF, extracted text, and source archive access.  The status-class
  consistency predicate is proved for every row of the finite catalogue.
  \uses{def:source-id}
\end{definition}
...
\begin{definition}[Typed source entry]
  ...
  class, and availability flags.  Its validity predicate checks nonempty and
  duplicate-free citation keys, kind/status consistency, and source-relative
  status paths; filesystem hashes remain under the external checker.
  \uses{def:source-id,def:source-kind,def:source-status-class,def:source-availability}
\end{definition}

% SOURCE: PROJECT_BOUNDARY.md, External inputs; docs/FORMALIZATION_SPEC.md:929--930.
...
\begin{definition}[Source inventory]
  ...
  \uses{def:source-id,def:source-kind,def:source-ref,def:source-entry,def:source-status-class}
\end{definition}
...
\begin{definition}[Source projection manifest]
  ...
  fields shared by Lean and JSON.  The inventory checker executes the Lean
  exporter and rejects any row-level drift before accepting the filesystem
  ledger.
  \uses{def:source-inventory,def:source-entry}
\end{definition}
...
\begin{definition}[External claim record]
  ...
  \uses{def:external-root-id,def:external-claim-class,def:inventory-valid-source-ref,def:source-entry}
\end{definition}
...
\begin{definition}[External claim ledger]
  ...
  \uses{def:external-claim-record,def:source-inventory}
\end{definition}
...
\begin{theorem}[Unique canonical claim references]
  ...
  \uses{def:external-claim-ledger,thm:external-claim-ledger-complete}
\end{theorem}
...
\begin{definition}[Catalogued external evidence]
  ...
  locator and optional evidence artifact.  Its artifact path must equal the
  canonical claim locator.  The Lean certificate checks the artifact's safe
  source-relative path and SHA-256 spelling.  The JSON checker then validates
  membership of that canonical path, filesystem existence, and equality of the
  checked-in file with the JSON digest; it does not automatically compare an
  arbitrary wrapper's digest text with that JSON value.  The wrapper does not
  type-check that its proposition is definitionally the one named by the
  future owner declaration.
  \uses{def:external-evidence,def:external-claim-ledger,def:external-claim-class,def:inventory-valid-source-ref,def:inventory-valid-evidence}
\end{definition}
...
\begin{definition}[External-claim projection manifest]
  ...
  source, optional locator artifact, owner, and Blueprint-label or enumerated
  source target.  The external checker rejects duplicate rows, owners, or
  targets; missing source coverage; unknown Blueprint/source targets; and any
  nonempty canonical locator artifact path that does not name a listed,
  required, existing regular file under the corresponding source directory.
  \uses{def:external-claim-ledger,def:source-projection}
\end{definition}
...
\begin{theorem}[Source coverage by claim roots]
  ...
  computation.
  \uses{def:external-claim-ledger,def:source-inventory,def:inventory-valid-source-ref}
\end{theorem}
