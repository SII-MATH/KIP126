#!/usr/bin/env python3
"""Shared derivation of a KIP126 PR's status from GitHub truth.
...
canonical `<!--kip126-scoreboard-->` comment's meta JSON), plus whether a review
is in flight right now (from the engine's `<!--kip126-review-in-progress-->`
...
REPO = os.environ.get("GH_REPO", "surenny/KIP126")

SCOREBOARD_MARKER = "<!--kip126-scoreboard-->"
...
_META_RE = re.compile(r"<!--kip126-meta:v1\s+(\{.*\})\s*-->", re.S)
# The engine's in-flight marker: `<!--kip126-review-in-progress {json}-->`, carrying a `head` and an
...
_INPROGRESS_RE = re.compile(r"<!--kip126-review-in-progress (.*?)-->", re.S)
