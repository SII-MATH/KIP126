...
| `review.yml` | Uses the `euler-review.request/v1` contract, `EULER_*` variables, exact current PR head, newest `scope`/`build`/`bump-guard`, strict `/review` authorization, a branch-reconciliation dispatch path, Euler idempotency, and initializer exclusion. It refreshes the status label after each dispatch outcome. | The protected webhook is never exposed to candidate code. The workflow never runs a model or reusable reviewer and preserves terminal exact-head semantic status. |
...
| `auto-merge.yml` | Uses the repository-local `scripts/merge_gate.py` on successful `pr-build`, verified scoreboard updates, or manual dispatch; it never runs a reviewer directly. | It reads exact-head KIP126 statuses and trusted scoreboard metadata, routes red `scope` to human review, enables native auto-merge for ready PRs, and updates only fully-green same-repository branches whose sole blocker is `behind`; no App credential or cross-repository backend is used. |
| `merge-sweep.yml` | Reconciles all open main-targeting PRs hourly or by dry-run dispatch through the same repository-local merge gate. This personal private repository has no merge queue, so the sweep repairs native auto-merge and stale same-repository branches rather than queue eviction. | It updates only a race-checked same-repository head after every exact-head policy gate is green, explicitly re-dispatches build/performance evidence, never rewrites forks or conflicts, and fails closed on ambiguous GitHub state. |
