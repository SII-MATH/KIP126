...
\begin{proposition}[Total differential identity for an arbitrary choice]
  ...
  $\theta_5$, the project proves the comparison
  \[
    \delta_1(h_6^2)=\lambda\eta\theta_5^2
  \]
  in the $\delta_1:S^{0,0}/\lambda\to S^{1,-1}$ total-extension problem.
  The external source-choice identity is used only as the base case for this
  comparison; the universal quantifier is an internal Section 7 obligation.
\end{proposition}

\begin{proof}
  Starting from the distinguished source choice supplied by
  \ref{thm:external-total-differential-identity}, compare the quadratic
  constructions for an arbitrary order-two choice and for the source choice.
  Use the order-two and torsion/choice-comparison evidence to show that their
  bottom-cell total differentials agree, then transport the source identity
  across this equality.  This proof must construct the comparison itself; it
  may not invoke a universal external identity.
\end{proof}
...
\begin{proof}
  ...
  The source-choice identity enters only through the internal proposition's
  proof, not as a universal external premise.
\end{proof}
