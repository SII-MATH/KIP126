...
\begin{proposition}[The $h_4$ classical--synthetic $d_2$ comparison]
  ...
  \lean{KIP126.Comparison.ClassicalSynthetic.SyntheticH₄DifferentialData}
  ...
  $(s,t,w)\mapsto(s,t)$.  The synthetic differential has the same first two
  degree shift, preserves $w$, and its target is the corresponding
  $\lambda\cdot h_0h_3^2$ representative.
\end{proposition}

\begin{proof}
  Consume the AIM-5 catalogue wrapper to obtain the classical differential and
  its $(1,16)\to(3,17)$ degree calculation.  The concrete reindexed comparison
  commutes with the page-$2$ differentials, while the synthetic presentation
  identifies the image target with multiplication by $\lambda$.  Since the
  synthetic shift is $(2,1,0)$, this target has tridegree $(3,17,16)$ and the
  weight is unchanged.
\end{proof}
