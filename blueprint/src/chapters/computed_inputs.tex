...
\section{Bulk import from the Lin proof database}

\begin{definition}[Pinned finite-page sphere differential table]
  \label{def:lin-proofs-sphere-table}
  \lean{KIP126.Computation.LinProofs.DifferentialRow,KIP126.Computation.LinProofs.RawData.lookup}
  \uses{def:lin-e2-catalogue}
  The importer scans all 2,672,275 rows of the \texttt{log} table of
  \texttt{proofs.db}, Zenodo 14875701, version \texttt{v126.3.cw49}.
  It exports 10,907 closed finite-page sphere differential records in
  86 shards, preserving database IDs and local additive-basis coordinates.
  Inverse records are normalized to source bidegrees.  A coverage manifest
  accounts separately for every unexported row; branches, unknowns,
  other spectra, extensions and sentinel pages are not silently asserted.
\end{definition}

\begin{definition}[Meaning on the fixed sphere sequence]
  \label{def:lin-proofs-sphere-meaning}
  \lean{KIP126.Computation.LinProofs.HasCoordinates,KIP126.Computation.LinProofs.DifferentialStatement}
  \uses{def:lin-proofs-sphere-table,def:h6-fixed-computation}
  Decode the two coordinate lists as sums of the literal CSV monomials,
  transport their homogeneous classes to the fixed sphere $E_2$, and require
  common-cycle representatives on $E_r$ satisfying the actual differential
  equation.  No nonvanishing on $E_r$ follows merely from a nonempty
  coordinate list.  The statement has no caller-supplied proposition field.
\end{definition}

\begin{theorem}[Uniform external soundness input for the sphere table]
  \label{thm:lin-proofs-sphere-sound}
  \lean{KIP126.Computation.LinProofs.sphereTable_sound,KIP126.Computation.LinProofs.differential_of_lookup,KIP126.Computation.LinProofs.row5541}
  \uses{def:lin-proofs-sphere-table,def:lin-proofs-sphere-meaning}
  \notready
  For every successful lookup in this fixed table, its decoded differential
  statement holds on the fixed sphere sequence.  This is one explicitly
  authorized external axiom, not one axiom per record and not a verification
  of the machine proof logs.  Database row 5541 is obtained by finite lookup
  and this same assumption.  Source hashes, admission rules, unsupported
  categories and reproduction commands are recorded in
  \texttt{docs/LIN\_PROOFS\_IMPORT.md}.
  Removing this axiom requires mathematical verification and comparison,
  not merely validating hashes.  It does not assert $h_6^2$ permanence.
\end{theorem}

\section{A reproducible low-degree $E_2$ example}
...
% LOCATORS: basis CSV IDs 0--15,17,19,20,22,23,35,42,50,51,401,2314;
% multiplication obtained by reduction with the relations CSV.
\begin{definition}[Low-degree table extracted from Lin's data]
  \label{def:lin-low-degree-table}
  \lean{KIP126.Examples.AdamsE2LowDegrees.table}
  \uses{def:adams-e2-basis-table}
  Extract the region $0\le s\le8$, $0\le t-s\le8$, together with
  $(1,64),(2,128),(1,16),(4,18),(5,20)$, from Lin's version
  \texttt{t261.2} of Zenodo record \texttt{7865526}.
  Retain the published additive-basis order and compute each covered basis
  product using the published polynomial relations.
  The resulting numerical table has 86 covered cells and 27 basis vectors:
  60 cells have dimension zero, 25 have dimension one, and $(5,20)$ has
  dimension two, with basis $(h_1d_0,h_0^4h_4)$ (source IDs 50 and 51).
  This construction imports numerical data; identifying it with the actual
  sphere page remains an explicit presentation input, not a consequence of
  parsing the file.  This dataset is distinct from the later near-$126$
  differential and extension catalogues below.
\end{definition}

\begin{lemma}[Low-degree table relations and a two-dimensional component]
  \label{lem:lin-low-degree-table-relations}
  \lean{KIP126.Examples.AdamsE2LowDegrees.h0_h1_zero,KIP126.Examples.AdamsE2LowDegrees.h1_h2_zero,KIP126.Examples.AdamsE2LowDegrees.h0_c0_zero,KIP126.Examples.AdamsE2LowDegrees.h1_cube,KIP126.Examples.AdamsE2LowDegrees.h6_square,KIP126.Examples.AdamsE2LowDegrees.page_two_products_sum_ne_zero}
  \uses{def:lin-low-degree-table,def:adams-e2-table-presentation}
  In the model algebra, the table implies
  $h_0h_1=h_1h_2=h_0c_0=0$, $h_1^3=h_0^2h_2$, and the specified
  square relation for $h_6$.
  For any presentation of this table on an existing page, the actual products
  $h_1d_0$ and $h_0^4h_4$ are its two specified basis vectors at $(5,20)$.
  Their sum has coordinates $(1,1)$ and is nonzero.
\end{lemma}
\begin{proof}
  \uses{lem:adams-e2-table-product}
  Evaluate the imported coefficients and impose the corresponding relations
  in the quotient algebra.  Under a presentation, transport these equalities
  to the page.  Applying the first basis-coordinate functional to the sum
  gives $1$, which rules out zero.
\end{proof}
