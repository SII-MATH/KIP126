...
% VERIFIED-IN: KIP126/Classical/Adams/Basic.lean, declarations
% KIP126.Classical.adamsOneLineDifferentials and cataloguedAdamsOneLine.
\begin{theorem}[$h_4$ $d_2$ tracer bullet]
  ...
  \lean{KIP126.Classical.adamsOneLineDifferentials}
  \lean{KIP126.Classical.Adams.cataloguedAdamsOneLine}
  \leanok
  For a chosen sphere $E_2$ presentation with its explicit internal
  multiplication, the located \texttt{adams\_one\_line} result supplies the
  relation
  \[
    d_2(h_4)=h_0h_3^2.
  \]
  The source is in bidegree $(1,16)$ and the target in $(3,17)$.  The wrapper
...
\begin{proof}
  The relation itself is an explicit hypothesis at the literature boundary.
  Package it with
  \texttt{KIP126.Classical.Adams.cataloguedAdamsOneLine}; the constructor fixes
  the root to \texttt{adams\_one\_line}, copies the canonical source reference
  from the claim ledger, and proves the wrapper's reference equality and
  supported trust class.  The internal relation uses the actual Mathlib
  $E_2$ differential component and the separately supplied sphere product.
\end{proof}
