...
% SOURCE: Burklund--Xu Construction 7.7, reference/BurklundXu/paper.txt:3022--3039;
% the project-level statement deliberately forgets the commutative-algebra
% structure and retains only the quotient maps and their triangle data.
\begin{proposition}[Coherence of quotient restrictions]
  ...
  \notready
  For every map of synthetic spectra $f:X\to Y$ and integers
  $0<k<n<m\le\infty$, the quotient restriction maps are natural:
  \[
    \rho^Y_{n,m}\circ(\nu f/\lambda^m)
      =(\nu f/\lambda^n)\circ\rho^X_{n,m}.
  \]
  ...
  \]
  Moreover, the nested quotient maps for every triple $0<k<n<m\le\infty$
  assemble with the canonical $\lambda$-power maps on the first and last
  terms into a morphism between the corresponding
  $\lambda$--$\rho$--$\delta$ cofiber triangles.  Its middle components are
  the identity on the largest quotient and $\rho_{k,n}$ on the smaller one;
  thus the multiplication and connecting maps commute with restriction,
  including the $m=\infty$ endpoint.  The same triangle compatibility is
  natural in $f$.  This proposition asserts no map of $E_\infty$- or
  $\mathrm{CAlg}$-objects.
\end{proposition}

\begin{proof}
  The quotient maps are induced by the nested cofibers of the natural maps
  $\lambda^m$, so functoriality of cofibers gives the displayed naturality
  square and the morphism of distinguished triangles.  The identity and
  composition equations are the equalities of the corresponding quotient
  projections; the same argument applies to the untruncated endpoint by
  taking the colimit along the quotient tower.  No multiplicative structure
  is used.
\end{proof}
