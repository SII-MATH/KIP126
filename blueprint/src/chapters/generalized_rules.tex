...
% SOURCE: aimpaper/main.tex:1848-1907, degree-uniqueness and rho-lift steps.
\begin{lemma}[Quotient lift gives source survival]
  \label{lem:mahowald-quotient-lift-survival}
  \uses{def:mahowald-input-data,thm:external-synthetic-einfty-quotient,prop:synthetic-einfty-map-compatibility}
  \notready
  For a Mahowald input package, a class
  $[\lambda^{l_1}x]\in
  \pi_{t-s-1,t-1+e(h)}(\nu X/\lambda^r)$ mapping under
  $\rho:\nu X/\lambda^r\to\nu X/\lambda^{r'-1}$ to the synchronized
  representative exists if and only if
  $x\in Z_{n+m+e(h)}^{s+l,t+l-1}(X)$.  In addition,
  $\lambda^{m_1}\bar y$ detects a unique class in
  $\pi_{t-s-1,t+r'-1}(\nu Z/\lambda^{m_1+1})$, modulo the explicit
  $B_{r'}^{s+n+l,t+n+l-1}(Y)$ indeterminacy after transport to $Y$.
\end{lemma}

\begin{proof}
  Substitute the two weights into the finite-quotient $E_\infty$ formula.
  Compatibility of $\rho$ identifies its image with the smaller cycle
  subgroup, which is exactly $Z_{n+m+e(h)}$.  At the top quotient weight, the
  next boundary group is zero in the stated range, giving the required
  uniqueness modulo the explicit classical indeterminacy.
\end{proof}
...
\begin{theorem}[Generalized Mahowald trick]
  ...
  \uses{def:mahowald-input-data,prop:normalized-synthetic-triangle,def:lambda-rho-delta-triangle,thm:external-may-smash-boundary,lem:mahowald-synchronized-representatives,lem:mahowald-quotient-lift-survival,lem:mahowald-lambda-division}
  \notready
  Let
...
\begin{proof}
  ...
  representative of $\bar x$; condition (4) supplies a compatible
  representative of $y$.  May's lemma produces a class mapping to both sides
  of the diagram.  The quotient-lift lemma turns its $\rho$ lift into the
  stronger survival statement for $x$.  Its other image is the normalized
  $f$ relation; divide by $\lambda^{l_1}$ using the final lemma to obtain the
  page extension with exactly $B_{r'}$ indeterminacy.
\end{proof}
...
\begin{proposition}[Cofiber regression for the Mahowald trick]
  ...
  \uses{thm:generalized-mahowald,def:hopf-cofiber-cell-notation,cor:classical-e2-no-crossing,def:external-evidence}
  \notready
...
\begin{proposition}[Certificate for loss of essentiality on an earlier page]
  ...
  on $E_{r'}$, where $2\le r'<r$ and $n\le r'-2+e(f)$.  Then there exist
  $0<a'\le n-e(f)$ and
...
% SOURCE: aimpaper/main.tex:2074-2081, Corollary cor:dfc6043e.
\begin{corollary}[Page-extension stretching]
  \label{cor:page-extension-stretch}
  \uses{prop:page-extension-loss-certificate}
  \notready
  Let $r\ge r'$ and suppose $d_n^{f,E_{r'}}(x)=y$, with $x$ surviving to
  $E_r$.  If no loss certificate of
  Proposition~\ref{prop:page-extension-loss-certificate} exists between
  $r'$ and $r$, then $d_n^{f,E_r}(x)=y$.
\end{corollary}

\begin{proof}
  If the extension failed or acquired only zero in its target coset at the
  later page, restrict the first later-page obstruction back to $E_{r'}$.
  The loss-certificate proposition would produce exactly the excluded source,
  shorter extension, and target.  Contraposition yields the stretched
  extension.
\end{proof}

% SOURCE: aimpaper/main.tex:2084-2099, Example exam:stretchext.
\begin{proposition}[Stretching the Hopf-map extension to $E_\infty$]
  \label{prop:page-stretch-regression}
  \uses{prop:mahowald-cofiber-regression,cor:page-extension-stretch,prop:page-crossing-regression,thm:external-synthetic-lambda-complete}
  \notready
  The $E_3$ extension
  $d_2^{\nu_{\mathrm{Hopf}},E_3}(h_0h_4^2)=h_0p$ has no loss certificate and
  stretches to
  $d_2^{\nu_{\mathrm{Hopf}},E_\infty}(h_0h_4^2)=h_0p$.
\end{proposition}

\begin{proof}
  The page-crossing regression exhausts the only degree-allowed shorter
  extension and excludes it by the Ext product relation.  The source is a
  permanent cycle in the required range.  Apply page-extension stretching for
  each finite page and pass to the compatible inverse limit.
\end{proof}
