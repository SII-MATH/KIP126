...
/-- KIP §0.2, Definition 0.30: The sphere spectrum 𝕊 is the unit of the
...
/-- Auxiliary: desuspension commutes with smash product.
Derived from `smashSuspIso` using the shift equivalence. -/
private noncomputable def smashDesuspIso (A B : 𝒮) :
    (shiftFunctor 𝒮 (-1 : ℤ)).obj A ⊗ B ≅ (shiftFunctor 𝒮 (-1 : ℤ)).obj (A ⊗ B) :=
  (shiftFunctorCompIsoId 𝒮 (1 : ℤ) (-1 : ℤ) (by omega)).symm.app _ ≪≫
  (shiftFunctor 𝒮 (-1 : ℤ)).mapIso (
    (ClosedSymmetricTensorTriangulated.smashSuspIso
      ((shiftFunctor 𝒮 (-1 : ℤ)).obj A) B).symm ≪≫
    (tensorRight B).mapIso
      ((shiftFunctorCompIsoId 𝒮 (-1 : ℤ) (1 : ℤ) (by omega)).app A) ≪≫
    eqToIso (by simp [Functor.id_obj, tensorRight]))

/-- Tensoring with a sphere implements the shift: Sⁿ ⊗ X ≅ Σⁿ X.
Proved by ℤ-induction on `smashSuspIso`. -/
noncomputable def sphere_tensor_shift (n : ℤ) (X : 𝒮) :
    (Sphere n : 𝒮) ⊗ X ≅ (shiftFunctor 𝒮 n).obj X :=
  Int.inductionOn' n 0
    -- Base case n = 0: Sphere 0 ⊗ X ≅ 𝕊 ⊗ X ≅ X ≅ Σ⁰X
    (((tensorRight X).mapIso ((shiftFunctorZero 𝒮 ℤ).app SphereSpectrum)) ≪≫
      (λ_ X) ≪≫
      ((shiftFunctorZero 𝒮 ℤ).app X).symm)
    -- Successor case n → n+1
    (fun k _ ih =>
      ((tensorRight X).mapIso ((shiftFunctorAdd 𝒮 k 1).app SphereSpectrum)) ≪≫
      ClosedSymmetricTensorTriangulated.smashSuspIso (Sphere k) X ≪≫
      (shiftFunctor 𝒮 1).mapIso ih ≪≫
      ((shiftFunctorAdd 𝒮 k 1).app X).symm)
    -- Predecessor case n → n-1
    (fun k _ ih =>
      have : k - 1 = k + (-1) := by omega
      this ▸
      (((tensorRight X).mapIso ((shiftFunctorAdd 𝒮 k (-1)).app SphereSpectrum)) ≪≫
      smashDesuspIso (Sphere k) X ≪≫
      (shiftFunctor 𝒮 (-1 : ℤ)).mapIso ih ≪≫
      ((shiftFunctorAdd 𝒮 k (-1)).app X).symm))

/-- KIP §0.2, Definition 0.30 (smash product): The smash product of spheres
satisfies S^m ⊗ S^n ≅ S^{m+n}. Derived from `sphere_tensor_shift`. -/
noncomputable def smashSpheres_iso (m n : ℤ) :
    (Sphere m : 𝒮) ⊗ (Sphere n : 𝒮) ≅ Sphere (m + n) :=
  sphere_tensor_shift m (Sphere n) ≪≫
  ((shiftFunctorAdd 𝒮 n m).app SphereSpectrum).symm ≪≫
  eqToIso (by change Sphere (n + m) = Sphere (m + n); congr 1; omega)

/-- The smash product of spheres: Sᵐ ⊗ Sⁿ ≅ Sᵐ⁺ⁿ. -/
noncomputable def smashSpheres (m n : ℤ) :
    (Sphere m : 𝒮) ⊗ (Sphere n : 𝒮) ≅ Sphere (m + n) :=
  smashSpheres_iso m n
...
/-- KIP §0.2, Definition 0.31 (mapping spectra): The mapping spectrum (internal
...
/-- KIP §0.2, Definition 0.31: The mapping spectrum represents morphisms:
πₙ F(X, Y) ≅ [Σⁿ X, Y].
